...
public class DOA {
    ...
    public static int getTotalPlayers() {
        ...
        try {
            ...
            java.sql.PreparedStatement pst = con.prepareStatement("SELECT COUNT(*) FROM USERS");
            java.sql.ResultSet rs = pst.executeQuery();
            if (rs.next()) {
                count = rs.getInt(1);
            }
        } catch (SQLException ex) {
            System.getLogger(DOA.class.getName()).log(System.Logger.Level.ERROR, "Error getting total players", ex);
        }
        return count;
    }
